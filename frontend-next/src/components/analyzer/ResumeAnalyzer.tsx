...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`bg-white border border-slate-200/80 shadow-lg shadow-slate-200/20 rounded-2xl overflow-hidden ${className}`}
    >
      {children}
    </div>
  );
...
function CardTitle({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <h3 className={`text-base font-bold flex items-center gap-2 ${className}`}>{children}</h3>;
}
...
function Badge({
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold border ${style} ${className}`}>
      {children}
    </span>
  );
}
...
type TabId = 'summary' | 'scores' | 'issues' | 'bullets';
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">
              Upload PDF &middot; Paste JD &middot; Get AI-powered insights
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col gap-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-purple-600">
                <Upload className="w-4 h-4" /> Upload Resume (PDF, max 2MB)
              </CardTitle>
            </CardHeader>
            <CardContent>
              <input
                ...
                {isUploading || isExtracting ? (
                  <div className="flex flex-col items-center gap-2 text-purple-600 animate-pulse">
                    ...
                    <span className="text-sm font-medium">
                      {isUploading ? 'Uploading...' : 'Extracting text from PDF...'}
                    </span>
                  </div>
                ) : resumeFile ? (
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-xs text-slate-400">
                      {extractedText
                        ? `Text extracted -- ${extractedText.split(/\s+/).length} words`
                        : 'Processing...'}
                    </span>
                    <button
                  ...
                  <div className="flex flex-col items-center gap-2 text-slate-400">
                    ...
                    <span className="text-sm font-medium">
                      ...
                      <span className="text-purple-500 underline">click to browse</span>
                    </span>
                  </div>
                )}
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-purple-600">
                <FileText className="w-4 h-4" /> Job Description (Optional)
              </CardTitle>
            </CardHeader>
            <CardContent>
              <textarea
          ...
          {aiResult && !aiLoading && (
            <Card className="flex-1 flex flex-col h-full bg-white">
              <CardHeader className="border-b border-slate-100 pb-4">
                <CardTitle className="text-purple-600">
                  <Cpu className="w-5 h-5" /> Analysis Results
                </CardTitle>
                <div className="flex flex-wrap justify-center gap-6 mt-4">
                  ...
                  <ScoreRing
                    score={aiResult.predicted_score_after_fixes}
                    label="After Fixes"
                    color="#10b981"
                  />
                </div>
              ...
              <div className="flex gap-1 p-2 bg-slate-50/50 border-b border-slate-100 overflow-x-auto">
                {(['summary', 'scores', 'issues', 'bullets'] as const).map((tab) => (
                  <button
                    key={tab}
                  ...
                  >
                    ...
                    {tab === 'issues' && aiResult.issues?.length > 0 && (
                      <span className="ml-1 px-1.5 py-0.5 rounded-md bg-red-500 text-white text-[9px] font-black">
                    ...
                    {!aiResult.issues?.length ? (
                      ...
                      aiResult.issues.map((iss, i) => (
                        <div key={i} className="p-3 bg-red-50 border border-red-100 rounded-xl">
                          ...
                          <p className="text-xs text-green-700 bg-green-50 p-2 rounded-lg mt-2">
                            {iss.suggested_fix}
                          </p>
                        </div>
                      ))
                  ...
                  <div className="space-y-4">
                    {!aiResult.rewritten_bullets?.length ? (
                      ...
                      aiResult.rewritten_bullets.map((b, i) => (
                        <div
                          key={i}
                          className="p-3 border border-slate-200 rounded-xl space-y-2 bg-slate-50"
                        >
                          ...
                          <div className="text-xs bg-purple-50 p-2 rounded-lg border border-purple-100">
                            <span className="font-black text-purple-700 block gap-1 flex items-center">
                              <Cpu className="w-3 h-3" /> Improved
                            </span>
