  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${
        styles[severity] || styles.nice_to_have
      }`}
    >
...
interface CareerInsightsProps {
  ...
  onApplyTailored: (data: ResumeData) => void;
}

export default function CareerInsights({ resumeData, onApplyTailored }: CareerInsightsProps) {
          ...
          <div className="space-y-4">
            ...
            {gapResult && (
              <div className="space-y-4 mt-4">
                        ...
                        <div key={i} className="p-4 border border-slate-100 rounded-xl">
                          ...
                          <div className="bg-emerald-50 p-3 rounded-lg border border-emerald-100">
                            <p className="text-xs text-emerald-800 font-medium">
                              Action: {gap.action_plan}
                            </p>
                            <p className="text-[10px] text-emerald-600 mt-1">
                              Estimated: {gap.time_estimate}
                            </p>
                          </div>
                        </div>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {gapResult.missing_skills.map((skill, i) => (
                        <span
                          key={i}
                          className="px-2 py-1 bg-red-50 text-red-700 border border-red-100 rounded-md text-xs font-medium"
                        >
                          {skill}
                        </span>
                      ))}
                ...
                {gapResult.recommended_projects && gapResult.recommended_projects.length > 0 && (
                  <div>
                        ...
                        <div key={i} className="p-3 bg-blue-50 border border-blue-100 rounded-xl">
                          ...
                          <div className="flex flex-wrap gap-1 mt-2">
                            {proj.skills_covered?.map((s, j) => (
                              <span
                                key={j}
                                className="text-[10px] px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded font-medium"
                              >
                                {s}
                              </span>
                            ))}
          ...
          <div className="space-y-4">
            ...
            {optimizeResult && (
              <div className="space-y-4 mt-4">
                ...
                {optimizeResult.keywords_to_include && optimizeResult.keywords_to_include.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {optimizeResult.keywords_to_include.map((kw, i) => (
                        <span
                          key={i}
                          className="px-2 py-1 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md text-xs font-medium"
                        >
                          {kw}
                        </span>
                      ))}
                ...
                {optimizeResult.optimized_sections?.experience &&
                        ...
                        {optimizeResult.optimized_sections.experience.map((exp, i) => (
                          <div key={i} className="p-3 border border-slate-100 rounded-xl">
                            ...
                            {exp.optimized_bullets?.map((bullet, j) => (
                              <div
                                key={j}
                                className="text-xs bg-purple-50 p-2 rounded-lg border border-purple-100 mb-1"
                              >
                                {bullet}
                              </div>
                            ))}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {optimizeResult.optimized_sections?.skills_to_add &&
                  ...
                  {optimizeResult.optimized_sections?.skills_to_emphasize &&
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {optimizeResult.optimized_sections.skills_to_emphasize.map((s, i) => (
                            <span
                              key={i}
                              className="text-[10px] px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded border border-blue-100"
                            >
                              {s}
                            </span>
                          ))}
          ...
          <div className="space-y-4">
            ...
            {atsResult && (
              <div className="space-y-4 mt-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2.5">
                      {Object.entries(atsResult.category_breakdown).map(([cat, data]) => {
                        const pct =
                          data.total > 0 ? Math.round((data.matched / data.total) * 100) : 0;
                        return (
                          <div key={cat}>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {atsResult.missing_keywords.map((kw, i) => (
                        <div
                          key={i}
                          className="flex items-start gap-2 p-2.5 border border-slate-100 rounded-lg"
                        >
                ...
                {atsResult.ats_tips && atsResult.ats_tips.length > 0 && (
          ...
          <div className="space-y-4">
            ...
            {tailorResult && (
              <div className="space-y-4 mt-4">
                ...
                {tailorResult.changes_summary && tailorResult.changes_summary.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {tailorResult.changes_summary.map((change, i) => (
                        <div
                          key={i}
                          className="p-3 border border-slate-100 rounded-xl flex items-start gap-2"
                        >
                          <span
                            className={`px-1.5 py-0.5 rounded text-[9px] font-black ${
                              change.change_type === 'reworded'
                                ? 'bg-blue-100 text-blue-700'
                                : change.change_type === 'added_keyword'
        ...
        {activeTab === 'cover-letter' && (
          <div className="space-y-4">
            ...
            <ActionButton
              ...
              disabled={!jobDescription.trim() || !companyName.trim()}
              loading={loading}
            ...
            {coverLetterResult && (
              <div className="space-y-4 mt-4">
                ...
                {coverLetterResult.key_points_highlighted &&
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {coverLetterResult.key_points_highlighted.map((pt, i) => (
                          <span
                            key={i}
                            className="px-2 py-1 bg-purple-50 text-purple-700 border border-purple-100 rounded-md text-xs font-medium"
                          >
                            {pt}
                          </span>
                        ))}
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-2">
                Recruiter Name <span className="text-slate-400 font-normal">(optional)</span>
              </label>
              <input
                type="text"
                value={recruiterName}
            ...
            {emailResult && (
              <div className="space-y-4 mt-4">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-800">
                        {emailResult.subject_line}
                      </p>
                    </div>
                    <button
                      onClick={() => handleCopy(emailResult.subject_line)}
                ...
                <button
                  onClick={() =>
                    handleCopy(
                      `Subject: ${emailResult.subject_line}\n\n${emailResult.email_body}`
                    )
                  }
                ...
                {emailResult.key_highlights_used && emailResult.key_highlights_used.length > 0 && (
                  <div>
